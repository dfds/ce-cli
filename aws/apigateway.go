...
var GetApiGatewayCmd = &cobra.Command{
	Use:   "apigateway",
	Short: "Get api gateways across all org accounts",
	Run: func(cmd *cobra.Command, args []string) {
		GetApiGateway(cmd, args)
	},
}

func GetApiGateway(cmd *cobra.Command, args []string) {
	...
	for id, creds := range assumedRoles {
		...
		go func(id string, creds *ststypes.Credentials, url string) {
			...
			color.Set(color.FgWhite)
			fmt.Printf(" Account %s (%s): Finding API gateways.\n", targetAccounts[id], id)
			sem.Acquire(ctx, 1)
			defer sem.Release(1)
			...
			for _, region := range regions {
				...
				pag := apigateway.NewGetRestApisPaginator(apiGatewayClient, &apigateway.GetRestApisInput{})
				data := []types.RestApi{}
				...
				for _, api := range data {
					fmt.Printf("%s (%s): %s - %s\n", id, region, *api.Name, *api.Id)
				}
			}
