...
var GetDynamoDbTableCmd = &cobra.Command{
	Use:   "dynamodb",
	Short: "Get api gateways across all org accounts",
	Run: func(cmd *cobra.Command, args []string) {
		GetDynamoDbTable(cmd, args)
	},
}

func GetDynamoDbTable(cmd *cobra.Command, args []string) {
	...
	for id, creds := range assumedRoles {
		...
		go func(id string, creds *ststypes.Credentials, url string) {
			...
			for _, region := range regions {
				...
				pag := dynamodb.NewListTablesPaginator(dynamoDbClient, &dynamodb.ListTablesInput{})
				data := []string{}
				for pag.HasMorePages() {
					...
					data = append(data, resp.TableNames...)
				}
